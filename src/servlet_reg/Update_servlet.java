...
public class Update_servlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String n=session.getAttribute("sid").toString();
			PrintWriter p=response.getWriter(); 
			
			Class.forName("com.mysql.cj.jdbc.Driver");
			Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/proj1","root","root");
			String sql=" update Student_account set name='"+fname+"',regno='"+regno+"',dob='"+dob+"',username='"+uname+"' where slno='"+id+"' and staffid='"+n+"' ";
			PreparedStatement stmt=con.prepareStatement(sql);
			 int i=stmt.executeUpdate();
			 if(i==1)
			 {
				 response.sendRedirect("Student_update.html");
			 }
		} catch (Exception e) {
